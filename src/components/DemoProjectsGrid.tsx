...
function DemoProjectCard({ product }: { product: DemoProduct }) {
  return (
    <motion.div
      ...
      className="group relative w-full md:w-[calc((100%-24px)/2)] lg:w-[calc((100%-48px)/3)] shrink-0 snap-start bg-zinc-950/90 rounded-3xl border border-zinc-800/90 hover:border-zinc-700 flex flex-col hover:-translate-y-1 transition-all duration-300 select-none shadow-2xl shadow-black/80 overflow-hidden"
    >
      {/* PREVIEW SCREENSHOT */}
      <a
        href={product.url}
      ...
      >
        ...
        <div className="absolute top-3 right-3 z-10 pointer-events-none">
          <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-black/80 backdrop-blur-md border border-zinc-800 text-white text-[10px] font-bold uppercase tracking-widest shadow-xl">
            Demo Live
          </span>
        </div>
      ...
      {/* BODY */}
      <div className="p-6 flex flex-col grow bg-zinc-950/90 text-white">
        <div className="mb-3">
          ...
          <ul className="space-y-1.5 grow">
            {product.features.slice(0, 3).map((f, i) => (
              <li key={i} className="flex items-start gap-2 text-xs text-zinc-400 font-normal">
                <FiCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-400" />
                {f}
              </li>
            ))}
          </ul>
        ...
        <div className="mt-5 pt-4 border-t border-zinc-800/80 flex justify-end">
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-white text-black font-black text-xs rounded-xl shadow-md hover:bg-zinc-200 transition-all cursor-pointer"
          >
            Esplora
