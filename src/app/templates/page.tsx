...
export default function TemplatesPage() {
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pt-24 pb-12 transition-colors duration-300">
      ...
      {/* Header Section */}
      <section className="max-w-7xl mx-auto px-6 mb-20 text-center relative z-10">
        <motion.div
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter mb-6 leading-tight">
            Parti Avvantaggiato con i nostri <br/>
            <span className="text-indigo-600 dark:text-indigo-400">Template Premium</span>
          </h1>
          <p className="max-w-2xl mx-auto text-lg text-slate-600 dark:text-slate-400 font-medium">
            Scegli una base solida e testata per il tuo prossimo progetto. Risparmia tempo e budget, senza rinunciare a prestazioni e design di altissimo livello.
          </p>
        </motion.div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
          {templates.map((tpl, idx) => (
            <motion.div
              ...
              className={`relative bg-white dark:bg-slate-900 rounded-3xl overflow-hidden border transition-all duration-300 hover:shadow-2xl flex flex-col ${
                tpl.popular 
            ...
            >
              ...
              {/* Immagine Mockup */}
              <div className="relative w-full aspect-video bg-slate-100 dark:bg-slate-800 group overflow-hidden border-b border-slate-100 dark:border-slate-800">
                <Image
                  src={tpl.image}
                ...
                <div className="absolute bottom-4 left-0 w-full flex justify-center px-4">
                   <span className="text-white text-[9px] sm:text-[10px] font-bold tracking-[0.15em] uppercase px-4 py-2 bg-slate-900/80 backdrop-blur-md rounded-full border border-white/20 shadow-xl text-center pointer-events-none">
                     Esempio Dimostrativo • 100% Personalizzabile
                   </span>
                </div>
              </div>
              ...
              <p className="text-sm md:text-base text-slate-600 dark:text-slate-400 font-medium leading-relaxed mb-8">
                {tpl.description}
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center">
                <Link
                  href={`/#pacchetti`}
                  className={`w-full text-center py-4 rounded-xl font-black uppercase tracking-widest text-xs text-white shadow-lg transition-all bg-linear-to-r ${tpl.color} hover:brightness-110`}
                >
                  Richiedi Template
                </Link>
      ...
      <section className="max-w-4xl mx-auto px-6 text-center pb-20">
        <motion.div
          ...
          viewport={{ once: true }}
          className="bg-indigo-600 rounded-3xl p-10 md:p-16 shadow-2xl shadow-indigo-600/20 relative overflow-hidden"
        >
          {/* Sfondi decorativi */}
          ...
          <div className="relative z-10">
            <h3 className="text-3xl md:text-4xl font-black text-white uppercase tracking-tighter mb-4">
              Non trovi il template giusto?
            </h3>
            <p className="text-indigo-100 text-sm md:text-base font-medium mb-8 max-w-xl mx-auto">
              Realizziamo anche soluzioni completamente personalizzate da zero. Raccontami il tuo progetto e troveremo la strada migliore per realizzarlo.
            </p>
            <Link
              href="/#contatti"
